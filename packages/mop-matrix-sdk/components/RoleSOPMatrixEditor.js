...
import { Modal } from 'mop-modal'; // ✅ CDN 导入
...
export class RoleSOPMatrixEditor {
    ...
    open(context, options = {}) {
        ...
        this.readOnly = options.readOnly || false;  // ✅ Support read-only mode
        this.isOpen = true;  // ✅ Mark as open
        this.loadData();
        this.render();

        console.log(`[RoleSOPMatrixEditor] Opened in ${this.readOnly ? 'READ-ONLY' : 'EDIT'} mode`);
    }

    // ✅ Helper method to get matrixData from Controller SSOT
    getMatrixData() {
        // 直接从 controller.model 读取，确保数据一致性
        return this.editor.controller.model.matrixData || { roles: {}, assignments: [] };
    }

    // ✅ Helper method to save matrixData to Controller SSOT
    saveMatrixData(matrixData) {
        // 直接更新 controller.model，不再序列化（EditorController.save() 会处理）
        this.editor.controller.model.matrixData = matrixData;
        console.log('[RoleSOPMatrixEditor] Matrix data saved to controller.model');
    }

    loadData() {
        // 从 controller.model.graphData 读取节点数据
        const activityNode = this.editor.controller.model.graphData.nodes.find(n => n.id === this.activityId);
            ...
            <div class="bg-white rounded-xl shadow-2xl w-[95vw] h-[90vh] flex flex-col overflow-hidden">
                ...
                <div class="px-6 py-4 border-t border-slate-200 bg-white flex justify-between items-center">
                    ...
                    <div class="flex gap-3">
                        ${this.readOnly ? `
                        <button id="close-btn-footer" class="px-8 py-2.5 bg-slate-600 hover:bg-slate-700 text-white rounded-lg transition-colors font-medium">关闭</button>
                        ` : `
    ...
    renderTaskCell(sopId, roleId, tasks) {
        ...
        return `
            <td class="matrix-cell">
                <div class="space-y-2">
                    ${tasks.map(task => `
                        <div class="group flex items-start gap-1 p-1 rounded ${!this.readOnly ? 'hover:bg-slate-100' : ''}">
    ...
    renderPICell(sopId, roleId, pis) {
        ...
        return `
            <td class="matrix-cell">
                <div class="space-y-2">
                    ${pis.map(pi => this.renderPIItem(pi, sopId, roleId)).join('')}
                    ${!this.readOnly ? `
    ...
    attachEvents() {
        ...
        // Footer buttons (conditional based on readOnly)
        const closeBtnFooter = this.overlay.querySelector('#close-btn-footer');
        if (closeBtnFooter) closeBtnFooter.onclick = () => this.close();
        ...
        this.overlay.querySelectorAll('[data-action="edit-pi"]').forEach(btn => {
            btn.onclick = (e) => {
                ...
                const matrixData = this.getMatrixData(); // Use getMatrixData
                console.log('🔧 [DEBUG] matrixData:', matrixData);

                const assignment = matrixData.assignments.find(a =>
    ...
    editPI(sopId, roleId, existingPI = null) {
        ...
        const drawer = new MetricFormDrawer({
            ...
            onSave: (piData) => {
                ...
                this.saveMatrixData(matrixData); // Save changes after PI modification
                this.refreshMatrix();
            }
        });
        drawer.open();
    ...
    addRole() {
        ...
        this.saveMatrixData(matrixData); // Save matrixData after adding role
    ...
    save() {
        // 1. 保存SOP到Activity节点（使用 controller.model）
        const activityNode = this.editor.controller.model.graphData.nodes.find(n => n.id === this.activityId);
        if (activityNode) {
            activityNode.sops = this.activity.sops;
        }

        // 2. matrixData 已经在每次修改时通过 saveMatrixData() 同步到 controller.model
        // 不需要再次赋值，EditorController.save() 会自动序列化

        // 3. 调用EditorView的save方法（内部会调用 controller.save()）
        this.editor.save();
